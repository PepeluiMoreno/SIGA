  ...
  <AppLayout :title="titulo" subtitle="Edita las metas, presupuesto, actividades y tareas predefinidos">
    ...
    <template v-else-if="plantilla">
      <div class="space-y-3">
        ...
        <!-- 1 · METAS -->
        <section :class="cardCls">
          <button type="button" @click="togglePanel('metas')" :class="accordionBtn(open.metas)">
            <span class="flex items-center gap-3">
              <span class="shrink-0 w-1.5 h-5 rounded-full bg-violet-500"></span>
              <h2 :class="titleCls">Metas predefinidas</h2>
            ...
            <ChevronDownIcon :class="chevronCls(open.metas)" />
          </button>
          <div v-show="open.metas" class="px-5 py-4 space-y-3">
        ...
        <!-- 2 · PRESUPUESTO -->
        <section :class="cardCls">
          <button type="button" @click="togglePanel('partidas')" :class="accordionBtn(open.partidas)">
            <span class="flex items-center gap-3">
              <span class="shrink-0 w-1.5 h-5 rounded-full bg-emerald-500"></span>
              <h2 :class="titleCls">Partidas presupuestarias</h2>
            ...
            <ChevronDownIcon :class="chevronCls(open.partidas)" />
          </button>
          <div v-show="open.partidas" class="px-5 py-4 space-y-3">
        ...
        <section :class="cardCls">
          <button type="button" @click="togglePanel('actividades')" :class="accordionBtn(open.actividades)">
            <span class="flex items-center gap-3">
              ...
              <h2 :class="titleCls">Actividades predefinidas</h2>
          ...
          <div v-show="open.actividades" class="px-5 py-4 space-y-4">
            <div v-for="(act, ai) in actividades" :key="act.id ?? ai"
              ...
              <div class="flex items-center gap-3 px-4 py-2.5 bg-sky-50/60 border-b border-slate-200">
                ...
                <div class="flex items-center gap-2 shrink-0">
                  <label class="text-xs text-slate-500">Días (offset):</label>
                  <input v-model.number="act.duracionDias" type="number" min="0"
                    class="w-16 h-7 px-2 text-xs border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-indigo-500 text-center"
...
import { PlusIcon, ChevronDownIcon, ChevronRightIcon, XMarkIcon } from '@heroicons/vue/24/outline'
...
const open = reactive({ metas: true, partidas: true, actividades: true })
function togglePanel(k) { open[k] = !open[k] }
...
async function cargar() {
  ...
  error.value = null
  try {
    const [dataP, dataT, dataH, dataN] = await Promise.all([
...
function agregarActividad() {
  const a = { id: null, nombre: '', descripcion: '', duracionDias: 0, orden: actividades.value.length + 1, tareas: [] }
  ...
  try {
    if (act.id) {
      await graphqlClient.request(ACTUALIZAR_PLANTILLA_ACTIVIDAD, {
        data: { actividadId: act.id, nombre: act.nombre.trim(), descripcion: act.descripcion || null, duracionDias: act.duracionDias ?? 0, orden: act.orden ?? 1 },
      })
    } else {
      const r = await graphqlClient.request(CREAR_PLANTILLA_ACTIVIDAD, {
        data: { plantillaId: plantilla.value.id, nombre: act.nombre.trim(), descripcion: act.descripcion || null, duracionDias: act.duracionDias ?? 0, orden: act.orden ?? 1 },
      })
