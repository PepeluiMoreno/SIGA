  ...
  <AppLayout title="Contactos" subtitle="Directorio de personas y entidades">
    ...
    <div class="flex flex-wrap gap-2 mb-4">
      ...
      <select v-model="filtroFaceta" class="px-3 py-2 border border-slate-300 rounded-md text-sm bg-white">
        <option value="TODOS">Todas las facetas</option>
      ...
      <table class="min-w-full divide-y divide-slate-200 text-sm">
        <thead class="bg-slate-50">
          <tr class="text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">
            ...
            <th class="px-4 py-3">Facetas vigentes</th>
          </tr>
        </thead>
          ...
          <tr v-for="c in contactosFiltrados" :key="c.id" class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3">
              <div class="flex flex-wrap gap-1">
                ...
                <span v-if="!facetasVigentes(c).length" class="text-xs text-slate-400">sin facetas</span>
              </div>
            </td>
          </tr>
        </tbody>
