...
class ReunionService:
    ...
    async def convocar_reunion(
        ...
        plataforma_telematica_id: Optional[UUID] = None,
    ...
    async def registrar_asistente(
        ...
        existente = await self.session.execute(
            select(AsistenteReunionSecretaria).where(
                and_(
                    ...
                    AsistenteReunionSecretaria.miembro_id == miembro_id,
                    AsistenteReunionSecretaria.eliminado == False,
                )
            )
        )
        ...
        asistente = AsistenteReunionSecretaria(
    ...
    async def listar_asistentes(self, reunion_id: UUID) -> List[AsistenteReunionSecretaria]:
        result = await self.session.execute(
            select(AsistenteReunionSecretaria).where(
                and_(
                    AsistenteReunionSecretaria.reunion_id == reunion_id,
