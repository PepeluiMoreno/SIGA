...
from ..models.convenio import TipoConvenio, Convenio, DelegacionFirma
...
class ConvenioService:
    ...
    async def _siguiente_referencia(self, anio: int) -> str:
        ...
        result = await self.session.execute(
            select(func.count(Convenio.id)).where(
                and_(
                    func.extract('year', Convenio.fecha_firma) == anio,
    ...
    async def registrar_convenio(
        ...
        convenio = Convenio(
            ...
            cif_contraparte=cif_contraparte,
    ...
    async def obtener_convenio(self, convenio_id: UUID) -> Optional[Convenio]:
        result = await self.session.execute(
            select(Convenio).where(
                and_(Convenio.id == convenio_id, Convenio.eliminado == False)
            )
        )
    ...
    async def listar_convenios(
        ...
        query = select(Convenio).where(Convenio.eliminado == False)
        if estado:
            query = query.where(Convenio.estado == estado)
        if tipo_convenio_id:
            query = query.where(Convenio.tipo_convenio_id == tipo_convenio_id)
        ...
        query = query.order_by(Convenio.fecha_inicio.desc())
        result = await self.session.execute(query)
        return list(result.scalars().all())
    ...
    async def cambiar_estado_convenio(
        ...
        if not convenio:
            raise ValueError(f"Convenio {convenio_id} no encontrado")
